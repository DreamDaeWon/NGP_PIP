...
CGoOneStageButton::CGoOneStageButton()
{
    Initailizer();
}
...
void CGoOneStageButton::Initailizer()
{
    HBITMAP* InBitMap{};
    TCHAR Name[50] = {};

    wsprintf(Name, L"../resource/Mainmenu/OneStageButton.bmp");
...
int CGoOneStageButton::Update(float fTime)
{
    m_fTime += fTime * 10.f;
    if (angle < 360) {
...
void CGoOneStageButton::LateUpdate(float fTime)
{
    ...
    if (CCollisionManager::GetInstance()->CheckCircle_And_Point(this, Mouse) && CKeyManager::GetInstance()->KeyDown(VK_LBUTTON))
    {
        ...
        Sleep(1300);
        CStageManager::GetInstance()->StageChanger(CStageManager::STAGE_ONE);
      
    }

}

void CGoOneStageButton::Render(HDC mDC)
{
...
float CGoOneStageButton::radian(float degrees)
{
